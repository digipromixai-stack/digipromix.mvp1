...
serve(async (req) => {
  ...
  // Content changed — gzip + upload + insert
  ...
  if (uploadError) {
    ...
    if (crawl_job_id) {
      await supabaseAdmin.from('crawl_jobs').update({ status: 'failed', error_message: `Storage upload failed: ${uploadError.message}`, completed_at: new Date().toISOString() }).eq('id', crawl_job_id)
    }
